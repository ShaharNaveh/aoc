...
def empty_spaces(universe: list[list[str]]) -> tuple[frozenset[int], frozenset[int]]:
    empty_xs = frozenset(
        {idx for idx, row in enumerate(universe) if "#" not in row}
    )
    empty_ys = frozenset(
...
def offseted_galaxy(
    ...
    x_offset = sum(
        offset - 1
        for _ in filter(lambda idx: pos.real > idx, empty_ys)
    )
    y_offset = sum(
        offset - 1
        for _ in filter(lambda idx: pos.imag > idx, empty_xs)
    )
...
def p1(puzzle_file):
    ...
    return sum(
        ...
        for pair in itertools.combinations(
            map(func, iter_galaxies(universe)), 2
        )
    )
