...
def run(memory: dict[int, int], inputs=None, transformer: callable = int, ip: int = 0):
    memory = memory.copy()
    inputs = iter(inputs)
    base = 0
    while True:
        ...
        match opcode:
            ...
            case OpcodeType.Input:
                dest = parameters[0]
                memory[dest] = transformer(next(inputs))
...
def p1(puzzle_file):
    ...
    return next(out for out in run(memory, inputs, transformer=ord) if out > 255)
